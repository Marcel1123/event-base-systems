...
public final class BrokerRegular extends AbstractBroker {
    ...
    static Queue<PublicationAvg> NbPublicationToPublish;
    public BrokerRegular(String brokerListenTopic) {
        super(brokerListenTopic);
        NBSubs = new ConcurrentHashSet<>();
        NbPublicationToPublish = new LinkedBlockingDeque<>();
        TB = new ConcurrentHashMap<>();
        Thread t1 = new Thread(new Runnable() {
            ...
            public void run() {
                while (true)
                {
                    ...
                    {
                        PublicationAvg pub_avg = NbPublicationToPublish.peek();
    ...
    public synchronized void process(Publication pub)
    {
        ...
        for (Subscription s: NBSubs)
        {
            if(s.match(pub))
            {
                System.out.println("Forward:" + pub +"\t reason: " + s);
                forward(pub);
            }
        }
    }
    ...
    {
        NbPublicationToPublish.add(pub_avg);
    }
